      ...
      <body class="bg-gray-50">
        <%@include file="navbar.jsp" %>
          ...
          <div class="bg-gradient-to-r from-orange-600 to-red-600 text-white py-12">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div class="flex justify-between items-center">
                ...
                <a href="/servlet-jsp-gr3/promotions?action=add"
          ...
          <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <c:if test="${not empty requestScope.promotions}">
                ...
                <div class="overflow-x-auto">
                  <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                      <c:forEach var="promo" items="${requestScope.promotions}">
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                          <td class="px-6 py-4 text-sm text-gray-700">
                            <fmt:formatDate value="${promo.datePromotionDebut}" pattern="dd/MM/yyyy" />
                          </td>
                          <td class="px-6 py-4 text-sm text-gray-700">
                            <fmt:formatDate value="${promo.datePromotionFin}" pattern="dd/MM/yyyy" />
                          </td>
                          <td class="px-6 py-4 text-sm font-semibold text-orange-600">
                            <fmt:formatNumber value="${promo.promotionValeur}" maxFractionDigits="0" />%
                          </td>
                          <td class="px-6 py-4 text-sm">
                            <jsp:useBean id="now" class="java.util.Date" />
                            <c:choose>
                              <c:when test="${promo.datePromotionDebut <= now && promo.datePromotionFin >= now}">
                                <span
                                  class="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-800">Active</span>
                              </c:when>
                              <c:when test="${promo.datePromotionDebut > now}">
                                <span
                                  class="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-800">Upcoming</span>
                              </c:when>
                              <c:otherwise>
                                <span
                                  class="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-800">Expired</span>
                              </c:otherwise>
                            </c:choose>
                          </td>
                          <td class="px-6 py-4 text-sm space-x-2">
                            <a href="/servlet-jsp-gr3/promotions?action=edit&id=${promo.id}"
                            ...
                            <a href="/servlet-jsp-gr3/promotions?action=delete&id=${promo.id}"
